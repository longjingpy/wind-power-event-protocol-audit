...
def legs(actual,schedule,price):
    signed=imbalance_cashflow(actual,schedule,price,price,.5)['net_imbalance_cost_gbp']
    return signed,np.maximum(signed,0),np.maximum(-signed,0)

def main():
    OUT.mkdir(parents=True,exist_ok=True)
    signed,debit,credit=legs(np.array([0,2,0,2]),np.ones(4),np.array([100,100,-100,-100]))
    ...
    for horizon in [1,2,4]:
        ...
        for split in ['validation','test']:
            ...
            for day,d in frame.groupby('day'):
                ...
                actual=d.actual_mw.to_numpy();price=d.systemBuyPrice.to_numpy()
                for model in MODELS:
                    ...
                    pre_signed,pre_gross,pre_credit=legs(actual,schedule,price)
                    for fraction in [0,.05,.1,.2]:
                        ...
                        post_signed,post_gross,post_credit=legs(np.array(delivered),schedule,price)
