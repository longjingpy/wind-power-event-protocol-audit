...
def main():
    ...
    test_daily=daily[daily.split.eq('test')];test_days=int(test_daily.day.nunique());test_intervals=int(test_daily.intervals.sum())
    (OUT/'verification.json').write_text(json.dumps(dict(status='PASS',gross_negative_price_hand_examples=True,
        ...
        test_complete_days=test_days,test_intervals=test_intervals,
