...
def compression():
    ...
    b.set(xlim=(-3.5,3.5),xticks=[-3,0,3],xlabel='PCA − raw ARI (×10⁻³)',title='b  Differences at full precision')
...
def sampling():
    ...
    fig,(a,b)=plt.subplots(1,2,figsize=(6.5,3.5),sharey=True);fig.subplots_adjust(left=.28,right=.98,top=.89,bottom=.19,wspace=.23)
    ...
    for i,(site,l,r) in enumerate(keys):
        ...
        b.text(.98,i,f'{q.matched_pairs.gt(0).sum()} cfg',fontsize=9,va='center',ha='right')
...
def paths():
    ...
    ax.set(xlabel='Shape coordinate',ylabel='Signed value');axis(ax);ax.legend(frameon=False,loc='lower right',ncol=1)
    ...
    fig,ax=plt.subplots(figsize=(6.5,2.65));fig.subplots_adjust(left=.13,right=.97,top=.91,bottom=.23)
...
def lidar():
    ...
    fig,(a,b)=plt.subplots(2,1,figsize=(6.5,5.15));fig.subplots_adjust(left=.14,right=.98,top=.95,bottom=.18,hspace=.60)
    pos=[];strings=[]
...
def localization():
    # Reuse archived plotted values; no change to experiment or labels.
    d=pd.read_csv(OLD/'fig8_event_localization.csv');print('localization columns',d.columns.tolist(),flush=True)
...
def economics():
    ...
    save(fig,'fig14_storage',15,['outputs/protocol_benchmark_v22/economics/'],
        'Storage correction capability under observed settlement prices over 58 complete test days. The benchmark observes the realised half-hour mean and applies a power- and SOC-constrained correction, including priced daily terminal-inventory restoration; it is an ex-post capability experiment. Top: gross debits. Bottom: signed settlement cost change at fixed forecast commitments, before degradation and capital costs. The 20%-power event-aware case reduces gross debit by 27.1%; this is delivery correction rather than forecast-error improvement.',panels=[('a',a),('b',b)])
