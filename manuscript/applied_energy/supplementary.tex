...
\subsection{Experiment map}\label{experiment-map}
...
\subsubsection{Table S0. Experiment map}\label{table-s0.-experiment-map}
...
\subsection{S1. Archive coverage and source eligibility}\label{s1.-archive-coverage-and-source-eligibility}
...
\subsection{S2. Detector configurations}\label{s2.-detector-configurations}
...
SDA uses the original boundary-crossing convention \citep{florita2013}. The conference OpSDA implementation merges eligible SDA segments by dynamic programming and a squared-length objective \citep{cui2015}. Door tolerance is 0.025 and amplitude threshold is 0.20.

\subsection{S3. Shapes, matching and summary units}\label{s3.-shapes-matching-and-summary-units}
...
\subsection{S4. Representation fitting and perturbation analysis}\label{s4.-representation-fitting-and-perturbation-analysis}
...
\subsection{S5. External representation evaluation}\label{s5.-external-representation-evaluation}

The Greek archive contains ten turbines observed during January-June 2020 \citep{greece2024}. Its twelve base configurations operate on ten-minute samples with lags of two, four and eight points. The physical lags are twenty, forty and eighty minutes. Four source observations on each side provide forty minutes of context.

SDWPF contains 134 turbines observed during 2020-2021 \citep{sdwpfdata}. The external pipeline aggregates power to thirty-minute bins and applies the twelve base configurations. Seven batches divide the turbine list for computation. The complete archive has 11,361,190 source records and 134 location entries.
...
\subsection{S6. Human-reviewed regions and operating logs}\label{s6.-human-reviewed-regions-and-operating-logs}

\subsubsection{Earlier focused-region review}\label{earlier-focused-region-review}
...
\subsubsection{Current eight-archive human reference}\label{current-eight-archive-human-reference}
...
Three actual identifiers supplied 320 assessments each. R\_147f4682 recorded 75 upward, 65 downward, 21 V, 22 inverted-V, 11 oscillatory, 75 quiet and 51 sustained-low regions (194 positive). R\_2c332025 recorded 64 upward, 50 downward, 27 V, 21 inverted-V, 34 oscillatory, 104 quiet and 20 sustained-low regions (196 positive). R\_29d247dd recorded 46 upward, 39 downward, 52 V, 64 inverted-V, 66 oscillatory, 42 quiet and 11 sustained-low regions (267 positive). All 960 responses were assessable. The exports supply region categories rather than start/end labels and are retained byte-for-byte. The earlier 120-region cohort remains separate.
...
\subsection{S7. Forecasting and storage objectives}\label{s7.-forecasting-and-storage-objectives}
...
\subsection{S8. Controlled episode localization}\label{s8.-controlled-episode-localization}
...
TimesNet, KAN-AD, TCN-AE and positional-Transformer-AE reconstruct the normal training sequences \citep{wu2023timesnet, zhou2025kanad}. Each run receives thirty full minibatch epochs and 750 parameter updates. Normal validation reconstruction selects a checkpoint. The three initialization seeds are 41, 42 and 43.
...
\subsection{S9. Weather and post-event context}\label{s9.-weather-and-post-event-context}

The regional ERA5 exposure is an absolute 100-m wind-speed change of at least 1.5 m/s over three hours \citep{hersbach2020, era5docs}. Weather timestamps use a fifteen-minute nearest-match tolerance. Outcomes record subsequent threshold-event starts within one, two or four hours.

The archived panel has 21,300 exposed and 84,761 unexposed records. The unadjusted risk differences are 0.0388, 0.0679 and 0.0863. NOAA station 580270 supplies a regional surface comparison with a ninety-minute match tolerance \citep{noaaisd}. Tables S4 and S5 show the observed risks and shifted-exposure contrasts. Table S15 adds 2,000 calendar-block resamples for 3-, 7- and 14-day blocks. Each resampled block carries all its turbine records, preserving within-block cross-turbine dependence. The main seven-day analysis occupies 31 ERA5 blocks and 27 NOAA blocks.
...
\subsection{S11. Policy-capacity storage experiment}\label{s11.-policy-capacity-storage-experiment}

The fixed baseline has battery power equal to 10\% of wind-farm nameplate capacity and energy equal to two hours of battery power. The source is Jiangsu's 2023 market-connection policy \citep{jiangsu2023storage}, applied here as a historical engineering scenario. The 2025 national tariff reform changes the role of compulsory storage \citep{ndrc2025market}. The Jiangsu rule supplies a historical engineering scenario. Yandun is an analogous Xinjiang archive, and the same technical design serves as an engineering comparator in this study.
...
\subsection{S12. Independent multirater collection}\label{s12.-independent-multirater-collection}
...
\subsection{S13. v18 protocol space and event hierarchy}\label{s13.-v18-protocol-space-and-event-hierarchy}
...
\subsection{S14. v18 common-support matrix and conditional information}\label{s14.-v18-common-support-matrix-and-conditional-information}

\subsubsection{Terminology and support units}\label{terminology-and-support-units}
...
\subsection{S15. v18 representation and physical-process probes}\label{s15.-v18-representation-and-physical-process-probes}
...
\subsection{S16. v18 Hill LiDAR confirmation and chronology}\label{s16.-v18-hill-lidar-confirmation-and-chronology}
...
\subsection{S17. v18 forecast-to-cost and storage replay}\label{s17.-v18-forecast-to-cost-and-storage-replay}
...
The v20 Jiangsu duplicated-slot screen is a historical scenario recorded under \path{outputs/protocol\_benchmark\_v20/economics/jiangsu\_counterfactual/}. It applies a medium/short-term allowance to simulated slots and does not evaluate the native ultra-short-term accuracy component. S22 supersedes it for that question using actual quarter-hour points and the 97\%/87\% rule. The French transfer design still requires historical RTE prices and a specified responsible-party perimeter. Table S55 separates these policy components.

\subsection{S18. Output and reproducibility map}\label{s18.-output-and-reproducibility-map}

The v18 event engine and unit-explicit economic functions are in \path{src/wind\_events/}. Structural, physical, LiDAR, threshold, weather, forecast and storage outputs are under \path{outputs/protocol\_benchmark\_v18/}. Independent SVG, PDF, PNG and data-table sources are under \path{manuscript/figures\_v18/}. The static support-aware benchmark index is under \path{outputs/protocol\_benchmark\_v18/leaderboard/}. The package wheel, isolated demo and unit tests are recorded under \path{outputs/protocol\_benchmark\_v18/package/}. The complete v18 input, transformation and evaluation record is \path{docs/V18\_CURRENT\_EXECUTION.md}. The v19 manifest records the multirater and SMARTEOLE stages. Current v21/v22 representation, sampling and economic tables remain under their versioned output folders. The v23 extension adds \path{outputs/protocol\_benchmark\_v23/cluster\_count\_common\_support.csv} and the \path{many\_to\_many/} directory: \texttt{configuration\_pairs.csv}, \texttt{site\_summary.csv}, \texttt{verification.json} and \texttt{coverage\_aggregation\_reconciliation.csv}. The last table verifies the equal-configuration versus pooled-coverage distinction against the original one-to-one counts. Supplementary Tables S57--S58 and Fig. S1 summarize these additions. The v24 native economic task adds Tables S59--S64, S22--S23 methods and the model-input replication bundle. The public v0.7.0 release mirrors aggregate outputs under \path{results/}, contains the current manuscript and editable Fig. 1, and links the de-identified processed-data release. The detailed local component ledger retains internal event clocks; public tables use aggregate counts. The SMARTEOLE support diagnostic remains \path{outputs/protocol\_benchmark\_v19/smarteole/diagnostic/report.json}, with the supported-row median, all-row and pair-weighted summaries.

\subsection{S19. Decision-aligned forecast extension}\label{s19.-decision-aligned-forecast-extension}

The extension fits residual power targets (future 30-min power minus issue-time power) using two feature arms: past weather and past weather plus frozen historical raw25 event features. The fixed candidate matrix crosses absolute- and squared-error losses, 7/15 leaf budgets, ramp weights 1/4 and persistence blends 0/0.25/0.5/0.75/1, giving 240 candidate predictions across 1-, 2- and 4-h horizons. Validation selects the primary nMAE candidate and a separate gross-debit candidate; no future prices or target power enter model actions. The test period is the previously used calendar and is labelled exploratory. Paired 3/7/14-day farm-block resampling supplies uncertainty. The 2-h event-aware selected model reduces gross debit by 16.51\% relative to the legacy historical-event ramp mixture, with a seven-day interval of 11.39--22.31\%; its nMAE reduction is 0.79 percentage points (0.40--1.22). Complete candidate rows, selections and effect intervals are under \path{outputs/protocol\_benchmark\_v20/economics/}.

\subsection{S20. Full settlement and online metered correction}\label{s20.-full-settlement-and-online-metered-correction}

The v22 economic extension is a separate 58-complete-day capability arm after the common-commitment forecast-only arm. It writes the battery action back to delivered power before applying the observed Elexon single-price settlement formula. Gross debit is the positive part of signed settlement cost, including negative-price periods; it is not the absolute sum of purchase and sale legs. The forecast-only arm keeps a persistence commitment, uses the issued forecast, current state of charge and a daily terminal-inventory constraint, and reports the capacity frontier for 0\%, 5\%, 10\% and 20\% of the 48.3-MW farm capacity. The separate ex-post capability arm uses the current half-hour power measurement in \texttt{planned\_inventory\_step}, includes settlement of the restoration actions required to return terminal inventory, and reports gross and signed settlement separately under the same 10--90\% state-of-charge bounds. The buy-side and sell-side price columns are both present and identical in this archived calendar. The metered balance allows negative delivery when charging exceeds wind generation; this represents grid import. Extra export headroom and grid charging are assumed engineering permissions, and terminal-restoration actions are settled. It evaluates 1-, 2- and 4-h schedules and retains 3-, 7- and 14-day block intervals. At the two-hour horizon, the 20\% battery paired with the selected weather-plus-event schedule reduces test gross debit from GBP 223,345 to GBP 162,784 (27.1\%; seven-day block interval 23.95--30.73\%) over 58 complete days; the accompanying signed settlement cashflow is GBP 57,195 before and GBP 71,075 after correction. Machine-readable outputs are \path{outputs/protocol\_benchmark\_v22/economics/capability\_daily.csv}, \texttt{capability\_frontier.csv} and \texttt{capability\_intervals.csv}.

\subsection{S21. Many-to-many correspondence and hierarchy}\label{s21.-many-to-many-correspondence-and-hierarchy}
...
\subsection{S22. Native-resolution rule-based accuracy-charge experiment}\label{s22.-native-resolution-rule-based-accuracy-charge-experiment}

\subsubsection{Data, operational clock and weather inputs}\label{data-operational-clock-and-weather-inputs}
...
The policy source is the official Jiangsu 2022 No.~53, Article 44(II), printed pages 19--20 \citep{jiangsu2022accuracy}. Its ultra-short-term point accuracy is 100(1-\textbar actual-forecast\textbar/capacity); the qualification thresholds are 97\% at 15 min and 87\% at four hours. A failed point costs CNY 4 per 10 MW, or CNY 34.98 at 87.45 MW. Equality at the threshold qualifies. The medium/short-term 2\% monthly allowance is not used for this distinct accuracy component. Costs aggregate the observed eligible points; exemptions, reporting failures and contractual purchase/sale income are outside this component. This is a 2022-rule simulation on the observed power calendar, not a reconstruction of a contemporaneous invoice.
...
\subsubsection{Reproduction and relation to earlier cost experiments}\label{reproduction-and-relation-to-earlier-cost-experiments}

Machine-readable results are under outputs/protocol\_benchmark\_v24/economics. Primary tables, all 3/7/14-day intervals, model-seed checks, ramp attribution and selection-objective comparisons are retained. The economic replication bundle includes de-identified model-input arrays, selected estimators, environment versions and six exact-output reproduction cases. It reproduces these model-level tests from transformed inputs; the private original-clock minute archive remains a distinct upstream preparation stage. The v20 duplicated-slot screen is superseded for the ultra-short-term question by this native point-level experiment. The historical screen remains a record of an earlier scenario, not evidence for the Article 44(II) charges.

\subsection{S23. British economic comparisons and the scope of monetary benefit}\label{s23.-british-economic-comparisons-and-the-scope-of-monetary-benefit}

\subsubsection{Preserved observed-price and metered-correction results}\label{preserved-observed-price-and-metered-correction-results}
...
\subsubsection{Forecast-only price-driven storage and complete trading income}\label{forecast-only-price-driven-storage-and-complete-trading-income}
...
\subsection{S24. Scalar-controlled polarity increments and noise sensitivity}\label{s24.-scalar-controlled-polarity-increments-and-noise-sensitivity}
...
\subsubsection{Anchor stability and measurement-noise experiments}\label{anchor-stability-and-measurement-noise-experiments}

Valid nonconstant shapes satisfy max\textbar x\textbar=1; the pre-normalization scale a=shape\_normalizer measures the original maximum deviation in training-production-scale units. Zero a is excluded before encoding. For additive representation perturbation e, define the opposing-extrema margin m=\textbar max(x)+min(x)\textbar. If m\textgreater2\textbar\textbar e\textbar\textbar∞, the dominant positive/negative sign cannot exchange under that perturbation. When noise is added to samples before subtracting the noisy start value, e includes that subtraction; the experiment implements it explicitly. This explains why opposing extrema close to equal magnitude, rather than a small normalized anchor, are the important sign-instability case.
...
A complementary absolute-noise test applies standard deviations 0.001/0.005/0.01 in the original training-production-scale units before normalization, on Pizhou and both LiDAR instruments. The effective normalized noise is sigma/a. Results are stratified by a\textless0.05, 0.05≤a\textless0.2, a≥0.2, and opposing-extrema margin below/above 0.1. Empty strata remain explicitly empty; notably the final T11 sample contains no a\textless0.05 events. These checks distinguish algorithmic instability near zero scale from the actual amplitude support of the evaluated populations.

\subsection{S25. Seven AI review sessions and source-separated reference agreement}\label{s25.-seven-ai-review-sessions-and-source-separated-reference-agreement}
...
\subsection{S26. Complete physical-process reconstruction}\label{s26.-complete-physical-process-reconstruction}

\subsubsection{Population, independent targets and chronology}\label{population-independent-targets-and-chronology}
...
For phase \(r_j=j/16\), the wind target is \(Y_j=u(s+r_j(e-s))-u(s)\), with j=0,\ldots,16. Linear interpolation occurs only within consecutive valid wind support. All trajectories start at zero. Full-trajectory error averages squared differences over j=1,\ldots,16 and over events before taking the square root. Geometry error applies the same calculation after subtracting each measured or predicted curve's own endpoint chord, \(R_j=Y_j-r_jY_{16}\). Thus this diagnostic asks whether intermediate evolution is recovered after removing endpoint tendency; it is not an error in absolute wind speed. The 10/30-min power tracks retain identical event identifiers, wind targets and splits. All scalar and temporal arms within a track use that track's power support.

\subsubsection{Scalar controls and representation matrix}\label{scalar-controls-and-representation-matrix}
...
The basic matrix contains C5, C19 and ten C19-plus-representation arms: raw25, raw/PCA6, raw/PCA12, GASF/PCA6, GASF/PCA5 plus sign, GADF/PCA6, dual angular fields, energy-balanced signed channels, and two angular/phase combinations. The Gramian angular difference field (GADF) follows the angular construction of Wang and Oates \citep{wang2015gaf}. With the same signed unit-domain x and \(c_i=\sqrt{1-x_i^2}\), \(D=cx^T-xc^T\) and \(D(-x)=-D(x)\). Because the event-start coordinate \(x_4=0\), the uncompressed row \(D_{4j}=x_j\). This is an exact full-field identity; compressed-coordinate performance is empirical. Dual fields combine GASF and GADF, each divided by its training root-mean-square vector energy. The signed-channel arm similarly balances GASF and raw coordinates. The phase arms combine three angular PCs with three orthonormal type-II discrete-cosine coordinates from the seventeen internal samples, or six of each. All standardization, energy factors and principal components are fitted on the corresponding training partition.
...
\subsubsection{Decoder selection and residual control}\label{decoder-selection-and-residual-control}
...
\subsubsection{Paired uncertainty and mechanistic diagnostics}\label{paired-uncertainty-and-mechanistic-diagnostics}
...
For duration standardization, each exact-duration stratum present in both power-defined groups receives mass equal to the smaller group count. An event in group g then has weight min(n\_compound,n\_primitive)/n\_g. These fixed, outcome-blind weights are combined with each shared calendar draw. The resulting compound-minus-primitive difference is descriptive effect modification, not an intervention on event topology (Table S78).
...
\subsubsection{Reproduction and result map}\label{reproduction-and-result-map}

The data interface is prepared by prepare\_\allowbreak{}process\_\allowbreak{}v26.py and prepare\_\allowbreak{}native\_\allowbreak{}process\_\allowbreak{}v26.py; physical\_\allowbreak{}process\_\allowbreak{}v26.py and physical\_\allowbreak{}calibration\_\allowbreak{}v26.py perform validation and locked evaluation. The final selection is final\_\allowbreak{}validation\_\allowbreak{}lock.json. Summary, hierarchy, duration-standardized and perturbation tables are produced by summarize\_\allowbreak{}process\_\allowbreak{}v26.py, process\_\allowbreak{}hierarchy\_\allowbreak{}v26.py, process\_\allowbreak{}duration\_\allowbreak{}control\_\allowbreak{}v26.py and process\_\allowbreak{}controls\_\allowbreak{}v26.py. Outputs reside under outputs/protocol\_\allowbreak{}benchmark\_\allowbreak{}v26. The six prepared tracks passed identical-event, target, split, finite-value and saved-prediction checks; targeted tests cover angular identities and continuous wind targets. Source calendars and the differing experiment denominators remain explicit.

\subsection{Supplementary result tables}\label{supplementary-result-tables}

\subsubsection{Table S1. Primary representation comparison (k = 4)}\label{table-s1.-primary-representation-comparison-k-4}
...
pizhou\_seen & event row permutation & 0.048 & 0.022 & 0.370 \\
pizhou\_unseen & event row permutation & 0.102 & 0.051 & 0.364 \\
...
pizhou\_seen & gaf pca6 & 0.275 & 0.248 & 0.560 \\
pizhou\_unseen & gaf pca6 & 0.313 & 0.268 & 0.568 \\
...
pizhou\_seen & gaf signed pca6 & 0.277 & 0.251 & 0.562 \\
pizhou\_unseen & gaf signed pca6 & 0.316 & 0.274 & 0.570 \\
...
pizhou\_seen & raw25 & 0.651 & 0.674 & 0.747 \\
pizhou\_unseen & raw25 & 0.682 & 0.684 & 0.751 \\
...
pizhou\_seen & raw pca6 & 0.648 & 0.669 & 0.744 \\
pizhou\_unseen & raw pca6 & 0.677 & 0.679 & 0.749 \\
...
pizhou\_seen & statistics9 & 0.514 & 0.493 & 0.746 \\
pizhou\_unseen & statistics9 & 0.537 & 0.509 & 0.726 \\
...
pizhou\_seen & within event time permutation & 0.536 & 0.648 & 0.727 \\
pizhou\_unseen & within event time permutation & 0.572 & 0.651 & 0.725 \\
Suining & within event time permutation & 0.513 & 0.612 & 0.732 \\
Yandun & within event time permutation & 0.438 & 0.580 & 0.687 \\
\end{longtable}
}

\subsubsection{Table S2. Raw25 block-length sensitivity}\label{table-s2.-raw25-block-length-sensitivity}
...
pizhou\_seen & 3 & 34 & 214524 & 0.651 {[}0.644, 0.684{]} \\
pizhou\_seen & 7 & 16 & 214524 & 0.651 {[}0.647, 0.683{]} \\
pizhou\_seen & 14 & 9 & 214524 & 0.651 {[}0.651, 0.677{]} \\
pizhou\_unseen & 3 & 34 & 60046 & 0.682 {[}0.672, 0.725{]} \\
pizhou\_unseen & 7 & 16 & 60046 & 0.682 {[}0.674, 0.728{]} \\
pizhou\_unseen & 14 & 9 & 60046 & 0.682 {[}0.676, 0.723{]} \\
\end{longtable}
}

\subsubsection{Table S3. Greek configuration-pair support strata}\label{table-s3.-greek-configuration-pair-support-strata}
...
\textgreater=100 & 42 & 0.669 & 0.385 & 0.961 \\
\end{longtable}
}

\subsubsection{Table S4. Descriptive weather contrasts}\label{table-s4.-descriptive-weather-contrasts}
...
\subsubsection{Table S5. Archived time-displacement diagnostics (4 h outcome)}\label{table-s5.-archived-time-displacement-diagnostics-4-h-outcome}
...
\subsubsection{Table S6. Scaled post-event power index and residual-power contrasts}\label{table-s6.-scaled-post-event-power-index-and-residual-power-contrasts}
...
\subsubsection{Table S7. Forecasts on common chronological targets}\label{table-s7.-forecasts-on-common-chronological-targets}
...
\subsubsection{Table S8. Storage costs under the legacy annual-charge assumptions}\label{table-s8.-storage-costs-under-the-legacy-annual-charge-assumptions}
...
\subsubsection{Table S9. Controlled synthetic event detection (IoU \textgreater= 0.3)}\label{table-s9.-controlled-synthetic-event-detection-iou-0.3}
...
\bottomrule\noalign{}
\endlastfoot
rate\_rule & 0 & default & 0.056 & 0.210 & 0.088 \\
rate\_rule & 0 & threshold only & 0.099 & 0.175 & 0.126 \\
rate\_rule & 0 & full protocol & 0.439 & 0.435 & 0.437 \\
mean\_rule & 0 & default & 0.255 & 0.752 & 0.381 \\
mean\_rule & 0 & threshold only & 0.364 & 0.568 & 0.444 \\
mean\_rule & 0 & full protocol & 0.742 & 0.740 & 0.741 \\
tcn\_ae & 41 & default & 0.118 & 0.802 & 0.206 \\
tcn\_ae & 41 & threshold only & 0.582 & 0.383 & 0.462 \\
tcn\_ae & 41 & full protocol & 0.575 & 0.667 & 0.618 \\
tcn\_ae & 42 & default & 0.118 & 0.810 & 0.206 \\
tcn\_ae & 42 & threshold only & 0.557 & 0.365 & 0.441 \\
tcn\_ae & 42 & full protocol & 0.584 & 0.657 & 0.619 \\
tcn\_ae & 43 & default & 0.117 & 0.807 & 0.204 \\
tcn\_ae & 43 & threshold only & 0.578 & 0.372 & 0.453 \\
tcn\_ae & 43 & full protocol & 0.567 & 0.662 & 0.611 \\
transformer\_ae & 41 & default & 0.064 & 0.640 & 0.116 \\
transformer\_ae & 41 & threshold only & 0.398 & 0.330 & 0.361 \\
transformer\_ae & 41 & full protocol & 0.833 & 0.425 & 0.563 \\
transformer\_ae & 42 & default & 0.059 & 0.652 & 0.109 \\
transformer\_ae & 42 & threshold only & 0.434 & 0.328 & 0.373 \\
transformer\_ae & 42 & full protocol & 0.879 & 0.417 & 0.566 \\
transformer\_ae & 43 & default & 0.060 & 0.645 & 0.110 \\
transformer\_ae & 43 & threshold only & 0.379 & 0.310 & 0.341 \\
transformer\_ae & 43 & full protocol & 0.828 & 0.432 & 0.568 \\
...
\subsubsection{Table S10. Human-review sampling support}\label{table-s10.-human-review-sampling-support}
...
\subsubsection{Table S11. Human-labelled centre-region detector agreement}\label{table-s11.-human-labelled-centre-region-detector-agreement}
...
\subsubsection{Table S12. Learned representations by farm, averaged over three seeds (k = 4)}\label{table-s12.-learned-representations-by-farm-averaged-over-three-seeds-k-4}
...
\subsubsection{Table S13. SDWPF learned-model transfer across seven batches and three seeds}\label{table-s13.-sdwpf-learned-model-transfer-across-seven-batches-and-three-seeds}
...
\subsubsection{Table S14. Matched-count-weighted agreement and detector-occupancy overlap}\label{table-s14.-matched-count-weighted-agreement-and-detector-occupancy-overlap}
...
\subsubsection{Table S15. Observational weather contrasts with shared calendar-block intervals}\label{table-s15.-observational-weather-contrasts-with-shared-calendar-block-intervals}
...
\subsubsection{Table S16. Event-level IoU and matching-strategy sensitivity}\label{table-s16.-event-level-iou-and-matching-strategy-sensitivity}
...
\subsubsection{Table S17. Yandun physical-horizon and sampling sensitivity}\label{table-s17.-yandun-physical-horizon-and-sampling-sensitivity}
...
\subsubsection{Table S18. Event-weight sensitivity under the legacy annual-charge assumptions}\label{table-s18.-event-weight-sensitivity-under-the-legacy-annual-charge-assumptions}
...
\subsubsection{Table S19. Adjusted weather association model}\label{table-s19.-adjusted-weather-association-model}
...
\subsubsection{Table S20. Greek local-training and frozen-transfer comparison}\label{table-s20.-greek-local-training-and-frozen-transfer-comparison}
...
\subsubsection{Table S21. Same-encoder Greek frozen and adapted TCN comparison}\label{table-s21.-same-encoder-greek-frozen-and-adapted-tcn-comparison}
...
\subsubsection{Table S22. 100-epoch upper-budget controlled detection sensitivity}\label{table-s22.-100-epoch-upper-budget-controlled-detection-sensitivity}
...
mean\_rule & calibrated & 0.742 & 0.740 & 0.741 & 0.000 \\
mean\_rule & default & 0.255 & 0.752 & 0.381 & 0.000 \\
rate\_rule & calibrated & 0.439 & 0.435 & 0.437 & 0.000 \\
rate\_rule & default & 0.056 & 0.210 & 0.088 & 0.000 \\
tcn\_ae & calibrated & 0.583 & 0.397 & 0.465 & 2425.000 \\
tcn\_ae & default & 0.068 & 0.542 & 0.121 & 2425.000 \\
timesnet & calibrated & 0.760 & 0.533 & 0.623 & 1841.667 \\
timesnet & default & 0.135 & 0.578 & 0.219 & 1841.667 \\
transformer\_ae & calibrated & 0.814 & 0.363 & 0.501 & 1975.000 \\
transformer\_ae & default & 0.050 & 0.499 & 0.091 & 1975.000 \\
\end{longtable}
}

\subsubsection{Table S23. Fixed 10\%-power and 2-hour storage costs normalized by test-calendar hours}\label{table-s23.-fixed-10-power-and-2-hour-storage-costs-normalized-by-test-calendar-hours}
...
\subsubsection{Table S24. Validation-selected storage designs across model-seed-price configurations}\label{table-s24.-validation-selected-storage-designs-across-model-seed-price-configurations}
...
\subsubsection{Table S25. Perfect-information and feasible online operating costs per installed MW}\label{table-s25.-perfect-information-and-feasible-online-operating-costs-per-installed-mw}
...
\subsubsection{Table S26. Exploratory test-grid cells with lower event-weighted storage cost}\label{table-s26.-exploratory-test-grid-cells-with-lower-event-weighted-storage-cost}
...
\subsubsection{Table S27. Controlled detection compute budget and model size}\label{table-s27.-controlled-detection-compute-budget-and-model-size}
...
\subsubsection{Table S28. Validation-selected detection hyperparameters and held-out performance}\label{table-s28.-validation-selected-detection-hyperparameters-and-held-out-performance}
...
Trans.-AE denotes the positional Transformer autoencoder. Size denotes TimesNet d\_model or KAN-AD Fourier order; width and z are encoder width and latent size for the sequence autoencoders. SD is across training seeds. Model selection uses mean validation F1; the simple rule selects its mean window on the same validation population.

\subsubsection{Table S29. Fresh post-selection synthetic confirmation}\label{table-s29.-fresh-post-selection-synthetic-confirmation}
...
\subsubsection{Table S30. Paired F1 differences from the validation-selected mean rule}\label{table-s30.-paired-f1-differences-from-the-validation-selected-mean-rule}
...
\subsubsection{Table S31. CPU score-generation cost of the selected configurations}\label{table-s31.-cpu-score-generation-cost-of-the-selected-configurations}
...
\subsubsection{Table S32. Matched and unmatched composition within detector-pair sides}\label{table-s32.-matched-and-unmatched-composition-within-detector-pair-sides}
...
\subsubsection{Table S33. Conditional raw25 agreement and retained support}\label{table-s33.-conditional-raw25-agreement-and-retained-support}
...
\bottomrule\noalign{}
\endlastfoot
Hill of Towie & amplitude\_abs & 151847/243684 & 0.206 & 0.226 & 0.992 & 0.814 & 0.855 \\
Hill of Towie & direction & 239933/243684 & 0.326 & 0.358 & 0.999 & 0.275 & 0.335 \\
Hill of Towie & duration\_hours & 131317/243684 & 0.178 & 0.196 & 0.998 & 0.837 & 0.853 \\
Hill of Towie & joint\_all\_four & 104335/243684 & 0.142 & 0.155 & 0.600 & 0.830 & 0.845 \\
Hill of Towie & power\_start & 214671/243684 & 0.291 & 0.320 & 0.995 & 0.633 & 0.660 \\
La Haute Borne & amplitude\_abs & 20627/34042 & 0.192 & 0.196 & 0.871 & 0.877 & 0.909 \\
La Haute Borne & direction & 33848/34042 & 0.315 & 0.322 & 0.787 & 0.173 & 0.225 \\
La Haute Borne & duration\_hours & 20098/34042 & 0.187 & 0.191 & 0.982 & 0.844 & 0.851 \\
La Haute Borne & joint\_all\_four & 14783/34042 & 0.138 & 0.141 & 0.097 & 0.964 & 0.973 \\
La Haute Borne & power\_start & 30788/34042 & 0.286 & 0.293 & 0.892 & 0.688 & 0.711 \\
Pizhou & amplitude\_abs & 169991/274570 & 0.197 & 0.208 & 0.992 & 0.837 & 0.876 \\
Pizhou & direction & 271655/274570 & 0.315 & 0.333 & 0.998 & 0.289 & 0.363 \\
Pizhou & duration\_hours & 151737/274570 & 0.176 & 0.186 & 0.998 & 0.843 & 0.865 \\
Pizhou & joint\_all\_four & 117670/274570 & 0.137 & 0.144 & 0.587 & 0.790 & 0.809 \\
Pizhou & power\_start & 238656/274570 & 0.277 & 0.293 & 0.992 & 0.638 & 0.673 \\
Suining & amplitude\_abs & 63990/102248 & 0.207 & 0.221 & 0.977 & 0.833 & 0.872 \\
Suining & direction & 100804/102248 & 0.325 & 0.349 & 0.994 & 0.296 & 0.359 \\
Suining & duration\_hours & 56265/102248 & 0.182 & 0.195 & 0.995 & 0.846 & 0.864 \\
Suining & joint\_all\_four & 44368/102248 & 0.143 & 0.154 & 0.372 & 0.847 & 0.859 \\
Suining & power\_start & 89315/102248 & 0.288 & 0.309 & 0.963 & 0.655 & 0.683 \\
Yandun & amplitude\_abs & 558155/811453 & 0.235 & 0.260 & 0.999 & 0.783 & 0.826 \\
Yandun & direction & 791342/811453 & 0.333 & 0.369 & 1.000 & 0.295 & 0.356 \\
Yandun & duration\_hours & 438801/811453 & 0.184 & 0.205 & 1.000 & 0.800 & 0.813 \\
Yandun & joint\_all\_four & 347316/811453 & 0.146 & 0.162 & 0.786 & 0.712 & 0.732 \\
Yandun & power\_start & 723809/811453 & 0.304 & 0.337 & 0.998 & 0.631 & 0.644 \\
\end{longtable}
}
...
\subsubsection{Table S34. Test-label oracle ceilings for adjacent-mean windows}\label{table-s34.-test-label-oracle-ceilings-for-adjacent-mean-windows}
...
\subsubsection{Table S35. Equal-validation noise calibration on a fresh high-noise draw}\label{table-s35.-equal-validation-noise-calibration-on-a-fresh-high-noise-draw}
...
\subsubsection{Table S36. Paired gains after source or target-noise calibration}\label{table-s36.-paired-gains-after-source-or-target-noise-calibration}
...
\subsubsection{Table S37. Validation-selected policy cost comparison}\label{table-s37.-validation-selected-policy-cost-comparison}
...
\subsubsection{Table S38. Representative validation-selected cost decomposition}\label{table-s38.-representative-validation-selected-cost-decomposition}
...
\subsubsection{Table S39. Real fixed 10\%-power/2-hour online and LP traces}\label{table-s39.-real-fixed-10-power2-hour-online-and-lp-traces}
...
pizhou & mse & low\_tail5000 & 2496.000 & 27.560 & 21.927 & -5.633 & 184 & 118 \\
...
pizhou & event weighted & low\_tail5000 & 2496.000 & 26.945 & 21.215 & -5.730 & 198 & 107 \\
yandun & mse & base & 914.000 & 15.225 & 15.219 & -0.006 & 84 & 84 \\
yandun & mse & high & 914.000 & 24.865 & 24.865 & 0.000 & 84 & 84 \\
yandun & mse & low\_tail5000 & 914.000 & 36.584 & 30.776 & -5.808 & 84 & 67 \\
yandun & event weighted & base & 914.000 & 19.627 & 19.620 & -0.006 & 84 & 90 \\
yandun & event weighted & high & 914.000 & 33.779 & 33.779 & 0.000 & 84 & 90 \\
yandun & event weighted & low\_tail5000 & 914.000 & 36.717 & 31.327 & -5.390 & 84 & 66 \\
\end{longtable}
}

\subsubsection{Table S40. Weather association model diagnostics}\label{table-s40.-weather-association-model-diagnostics}
...
\subsubsection{Table S41. All neural model-size validation and test curves}\label{table-s41.-all-neural-model-size-validation-and-test-curves}
...
\bottomrule\noalign{}
\endlastfoot
TimesNet & h08\_z08 & 0.681 & 0.033 & 0.631 & 0.024 \\
TimesNet & h16\_z16 & 0.662 & 0.013 & 0.609 & 0.014 \\
TimesNet & h32\_z16 & 0.632 & 0.008 & 0.615 & 0.007 \\
KAN-AD & h08\_z08 & 0.737 & 0.085 & 0.710 & 0.096 \\
KAN-AD & h16\_z16 & 0.824 & 0.065 & 0.795 & 0.097 \\
KAN-AD & h32\_z16 & 0.796 & 0.018 & 0.751 & 0.015 \\
TCN-AE & sequence\_h08\_z08 & 0.757 & 0.009 & 0.729 & 0.007 \\
TCN-AE & sequence\_h16\_z16 & 0.660 & 0.006 & 0.618 & 0.012 \\
TCN-AE & sequence\_h32\_z16 & 0.647 & 0.022 & 0.601 & 0.021 \\
Transformer-AE & sequence\_h08\_z08 & 0.718 & 0.021 & 0.660 & 0.019 \\
Transformer-AE & sequence\_h16\_z16 & 0.634 & 0.002 & 0.587 & 0.009 \\
Transformer-AE & sequence\_h32\_z16 & 0.609 & 0.015 & 0.560 & 0.012 \\
\end{longtable}
}

\subsubsection{Table S42. All adjacent-mean candidate curves}\label{table-s42.-all-adjacent-mean-candidate-curves}
...
\subsubsection{Table S43. Yandun raw25 agreement across native and aggregated resolutions}\label{table-s43.-yandun-raw25-agreement-across-native-and-aggregated-resolutions}
...
\subsubsection{Table S44. Received human-reference morphology by archive}\label{table-s44.-received-human-reference-morphology-by-archive}

Three observers; central two-hour regions displayed with four hours of context. Observer 1 = R\_147f4682; Observer 2 = R\_29d247dd; Observer 3 = R\_2c332025. V and inverted V are listed separately; quiet and sustained-low are presence-negative.
...
\subsubsection{Table S45. Event-presence performance of all fixed detectors on the three-observer reference}\label{table-s45.-event-presence-performance-of-all-fixed-detectors-on-the-three-observer-reference}
...
\subsubsection{Table S46. Three-observer detector F1 sensitivity to calendar-block length}\label{table-s46.-three-observer-detector-f1-sensitivity-to-calendar-block-length}
...
\subsubsection{Table S47. Prespecified detector by archive}\label{table-s47.-prespecified-detector-by-archive}
...
\subsubsection{Table S48. Independent partition-metric sensitivity}\label{table-s48.-independent-partition-metric-sensitivity}
...
hill\_2021 & gaf bit6 & 112 & 0.206 & 0.452 & 1.970 & 0.208 \\
hill\_2021 & gaf pca6 & 112 & 0.207 & 0.451 & 1.980 & 0.209 \\
hill\_2021 & raw25 & 112 & 0.763 & 0.913 & 0.399 & 0.763 \\
...
\subsubsection{Table S49. Signed-weather log-loss decomposition}\label{table-s49.-signed-weather-log-loss-decomposition}
...
\subsubsection{Table S50. Chronologically calibrated LiDAR direction discrimination}\label{table-s50.-chronologically-calibrated-lidar-direction-discrimination}
...
\subsubsection{Table S51. All forecast variants under observed Elexon prices}\label{table-s51.-all-forecast-variants-under-observed-elexon-prices}
...
\subsubsection{Table S52. Common-commitment capacity and operating-cost decomposition}\label{table-s52.-common-commitment-capacity-and-operating-cost-decomposition}
...
\subsubsection{Table S53. Detector-perturbation axes}\label{table-s53.-detector-perturbation-axes}
...
\subsubsection{Table S54. Independent human agreement before adjudication}\label{table-s54.-independent-human-agreement-before-adjudication}
...
\subsubsection{Table S55. Jurisdiction-specific settlement requirements used to design transfer simulations}\label{table-s55.-jurisdiction-specific-settlement-requirements-used-to-design-transfer-simulations}
...
Policy quantity
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Requirement used in simulation design
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Current evidence
\end{minipage} \\
\midrule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
Jiangsu & Medium-short forecast & 15-min curve; 96-point assessment; next-day \textgreater=90\%, day-10 \textgreater=70\% & Official Jiangsu No.~53 (2022) \\
...
\subsubsection{Table S56. Decision-aligned forecast extension at the two-hour horizon}\label{table-s56.-decision-aligned-forecast-extension-at-the-two-hour-horizon}
...
\subsubsection{Table S57. Cluster-count sensitivity on frozen test populations}\label{table-s57.-cluster-count-sensitivity-on-frozen-test-populations}
...
k
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Representations in support intersection
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
N
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Raw25 ARI
...
hill & 6 & 1 & 105 & 0.602 & --- & --- \\
hill\_2021 & 2 & 6 & 91 & 0.904 & 0.233 & 0.144 \\
hill\_2021 & 4 & 6 & 112 & 0.711 & 0.492 & 0.206 \\
hill\_2021 & 6 & 6 & 112 & 0.587 & 0.651 & 0.252 \\
...
\subsubsection{Table S58. Many-to-many correspondence at IoU 0.5}\label{table-s58.-many-to-many-correspondence-at-iou-0.5}
...
hill\_2021 & 370,583 & 33.4 & 17.8 & 31.9 & 36.2/40.8 \\
...
\subsubsection{Figure S1. Split and merge structure in the overlap graph}\label{figure-s1.-split-and-merge-structure-in-the-overlap-graph}

\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Primitive and hierarchy-inclusive correspondence at IoU 0.5. Each segment gives the fraction of overlap components with one-to-one, one-to-many, many-to-one or many-to-many topology. All 136 configuration pairs are retained in each population.}]{supp_overlap_topology.pdf}}
\caption{Primitive and hierarchy-inclusive correspondence at IoU 0.5. Each segment gives the fraction of overlap components with one-to-one, one-to-many, many-to-one or many-to-many topology. All 136 configuration pairs are retained in each population.}
\end{figure}

\Needspace{17\baselineskip}\subsubsection{Table S59. Primary policy-fee results and strong controls}\label{table-s59.-primary-policy-fee-results-and-strong-controls}
...
\subsubsection{Table S60. Paired calendar-block uncertainty for policy-fee comparisons}\label{table-s60.-paired-calendar-block-uncertainty-for-policy-fee-comparisons}
...
\subsubsection{Table S61. Classifier-seed robustness with fixed transformations and selected settings}\label{table-s61.-classifier-seed-robustness-with-fixed-transformations-and-selected-settings}
...
\Needspace{17\baselineskip}\subsubsection{Table S62. Selection-objective comparison}\label{table-s62.-selection-objective-comparison}
...
\subsubsection{Table S63. Full-calendar attribution of rule-based charges}\label{table-s63.-full-calendar-attribution-of-rule-based-charges}
...
\subsubsection{Table S64. Complete-income British test outcomes}\label{table-s64.-complete-income-british-test-outcomes}
...
\subsubsection{British large-ramp exposure concentration}\label{british-large-ramp-exposure-concentration}

\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Earlier British large-ramp exposure concentration. Grey bars show interval share and magenta bars the share of persistence gross debits.}]{fig12_ramp_exposure.pdf}}
\caption{Earlier British large-ramp exposure concentration. Grey bars show interval share and magenta bars the share of persistence gross debits.}
\end{figure}

\subsubsection{British forecast-cost comparison}\label{british-forecast-cost-comparison}

\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Earlier British forecast-error and gross-debit comparison, on common targets within each horizon. This gross-exposure task is distinct from complete trading income in Table S64.}]{fig13_forecast_cost.pdf}}
\caption{Earlier British forecast-error and gross-debit comparison, on common targets within each horizon. This gross-exposure task is distinct from complete trading income in Table S64.}
\end{figure}

\subsubsection{British metered-correction capability}\label{british-metered-correction-capability}

\begin{figure}[H]
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Earlier ex-post metered-correction capability. The event-aware 20\% battery reduces gross debits by 27.1\% while signed settlement cost rises. Realized half-hour power, inventory restoration and model-specific schedules define this task; it is not an issue-time forecast or net-profit gain.}]{fig14_storage.pdf}}
\caption{Earlier ex-post metered-correction capability. The event-aware 20\% battery reduces gross debits by 27.1\% while signed settlement cost rises. Realized half-hour power, inventory restoration and model-specific schedules define this task; it is not an issue-time forecast or net-profit gain.}
\end{figure}

\subsubsection{Table S65. Physical factorial evaluation populations}\label{table-s65.-physical-factorial-evaluation-populations}
...
\subsubsection{Table S66. Direction AUROC in the common-input factorial}\label{table-s66.-direction-auroc-in-the-common-input-factorial}
...
\subsubsection{Table S67. Paired increments of S+G5+b}\label{table-s67.-paired-increments-of-sg5b}
...
\subsubsection{Table S68. Paired contrasts for the original LiDAR headline models}\label{table-s68.-paired-contrasts-for-the-original-lidar-headline-models}
...
\subsubsection{Table S69. Direction-AUROC controls for dimension, sign and anchor}\label{table-s69.-direction-auroc-controls-for-dimension-sign-and-anchor}
...
\subsubsection{Table S70. Anchor flips under relative representation-channel noise}\label{table-s70.-anchor-flips-under-relative-representation-channel-noise}
...
\subsubsection{Table S71. Small-amplitude and opposing-extrema strata}\label{table-s71.-small-amplitude-and-opposing-extrema-strata}
...
\subsubsection{Table S72. Source-separated reference agreement}\label{table-s72.-source-separated-reference-agreement}
...
\subsubsection{Table S73. Pairwise agreement by source combination}\label{table-s73.-pairwise-agreement-by-source-combination}
...
\subsubsection{Table S74. Protected anchor sign and scalar endpoint direction}\label{table-s74.-protected-anchor-sign-and-scalar-endpoint-direction}
...
\Needspace{17\baselineskip}\subsubsection{Table S75. Unique-interval support for measured-wind reconstruction}\label{table-s75.-unique-interval-support-for-measured-wind-reconstruction}
...
{\def\LTcaptype{none} % do not increment counter
\begingroup\AtBeginEnvironment{longtable}{\footnotesize}\setlength{\tabcolsep}{3pt}
\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
...
\Needspace{17\baselineskip}\subsubsection{Table S76. All-event wind-process error reductions}\label{table-s76.-all-event-wind-process-error-reductions}
...
\Needspace{17\baselineskip}\subsubsection{Table S77. Power-defined hierarchy and full-wind-trajectory recovery}\label{table-s77.-power-defined-hierarchy-and-full-wind-trajectory-recovery}
...
\Needspace{17\baselineskip}\subsubsection{Table S78. Common-duration standardization of the full-trajectory contrast}\label{table-s78.-common-duration-standardization-of-the-full-trajectory-contrast}
...
\Needspace{17\baselineskip}\subsubsection{Table S79. Ordered-correspondence controls for internal wind geometry}\label{table-s79.-ordered-correspondence-controls-for-internal-wind-geometry}
...
\subsubsection{Table S81. Conditional information with the implemented pre-window-power control}\label{table-s81.-conditional-information-with-the-implemented-pre-window-power-control}
...
gaf\_pca6 & 268,569/274,569 & 97.8\% & 0.436 & 0.404 & 0.862 \\
gaf\_bit6 & 268,569/274,569 & 97.8\% & 0.436 & 0.406 & 0.862 \\
\end{longtable}
\endgroup
}

\subsubsection{Table S82. Seven-day calendar-block contributions and leave-one-block-out diagnostics}\label{table-s82.-seven-day-calendar-block-contributions-and-leave-one-block-out-diagnostics}

Each row uses the frozen validation-selected models and the same event targets. Ordinary rows give one seven-day block; leave\_out rows remove that block. These rows describe temporal concentration in the evaluated period and are not additional independent weather replications.
...
T11 & leave\_out\_1 & 439 & 134 & 1.344 & 1.321 & 1.72\% \\
T11 & leave\_out\_2 & 294 & 97 & 1.553 & 1.379 & 11.24\% \\
T11 & leave\_out\_3 & 329 & 121 & 1.673 & 1.606 & 3.98\% \\
T11 & leave\_out\_4 & 459 & 143 & 1.447 & 1.411 & 2.53\% \\
T07 & 1 & 3 & 1 & 0.935 & 0.909 & 2.81\% \\
T07 & 2 & 68 & 15 & 1.183 & 1.205 & -1.87\% \\
T07 & 3 & 82 & 16 & 0.813 & 0.736 & 9.55\% \\
T07 & 4 & 17 & 6 & 1.322 & 1.091 & 17.48\% \\
T07 & leave\_out\_1 & 167 & 37 & 1.036 & 0.989 & 4.50\% \\
T07 & leave\_out\_2 & 102 & 23 & 0.921 & 0.811 & 11.97\% \\
T07 & leave\_out\_3 & 88 & 22 & 1.204 & 1.175 & 2.39\% \\
T07 & leave\_out\_4 & 153 & 32 & 0.997 & 0.976 & 2.13\% \\
...
SMARTEOLE & leave\_out\_1 & 445 & 42 & 1.350 & 1.327 & 1.77\% \\
SMARTEOLE & leave\_out\_2 & 259 & 31 & 1.059 & 1.028 & 2.97\% \\
SMARTEOLE & leave\_out\_3 & 277 & 17 & 1.446 & 1.422 & 1.69\% \\
SMARTEOLE & leave\_out\_4 & 366 & 36 & 1.438 & 1.420 & 1.22\% \\
\end{longtable}
\endgroup
}

\subsubsection{Table S83. Absolute RMSE and relative gains for full wind-process reconstruction}\label{table-s83.-absolute-rmse-and-relative-gains-for-full-wind-process-reconstruction}
...
\bottomrule\noalign{}
\endlastfoot
lidar\_T11 & all & 507 & 1.492 & 1.425 & 4.50\% & 1.183 & 1.052 & 11.09\% \\
lidar\_T11 & compound & 165 & 1.729 & 1.527 & 11.67\% & 1.414 & 1.248 & 11.70\% \\
lidar\_T11 & primitive & 342 & 1.363 & 1.373 & -0.72\% & 1.054 & 0.942 & 10.56\% \\
lidar\_T07 & all & 170 & 1.034 & 0.988 & 4.47\% & 0.841 & 0.615 & 26.94\% \\
lidar\_T07 & compound & 38 & 1.092 & 0.825 & 24.45\% & 1.003 & 0.651 & 35.12\% \\
lidar\_T07 & primitive & 132 & 1.016 & 1.030 & -1.30\% & 0.789 & 0.604 & 23.44\% \\
