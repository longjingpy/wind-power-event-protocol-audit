...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.10}}
...
Compound RMSE reduction (\%)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
All-event geometry reduction (\%)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig13_physical_process.pdf}
\caption{Ordered trajectories recover intermediate wind-process structure. Top: outcome-blind representative compound events selected by median power-defined duration and amplitude, showing measured wind change, the scalar control and the ordered trajectory. Bottom: absolute full-curve RMSE for compound and primitive-only events under the same native 10-min inputs. The complete matrix, event counts and calendar-block diagnostics are provided in Supplementary Tables S80 and S82.}
\label{fig:main10}
\end{figure}
