...
The harmonized catalogue contains 4,542,258 candidate intervals and 3,226,925 complete 25-coordinate shapes from seven archives and 333 turbines. Basic detector intervals and composite V-shaped or inverted-V episodes form separate event levels. Seventeen detector configurations share a 30-min grid, physical lags of 60, 120 and 240 min, a 24-h historical window and two hours of context on each side. Table 1 summarizes the archive scale before matching; Fig.~\ref{fig:main2} applies these rules to the same measured production trajectory, making protocol-dependent event boundaries visible before the aggregate comparison.
...
\textbf{Table 2. Structural survival and catalogue coverage}
...
For the signed-domain Gramian angular summation field (GAF) used here, a path and its sign reversal produce the same image. Fig.~\ref{fig:main7} shows the paired paths and their identical angular field in one mechanism figure. This identity gives a precise intervention: one sign bit at a nonzero reference coordinate resolves the ambiguity of the full field. For a compact representation, we retain five coordinates from principal component analysis (PCA) and reserve a sixth coordinate for polarity. This protects the directional distinction through compression; exact recovery of the entire path is a property of the full field plus sign, whereas the compressed representation is evaluated empirically. The next experiment tests the physical value of retaining that distinction.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig07_polarity_mechanism.pdf}
\caption{A protected polarity coordinate preserves a distinction erased by angular encoding. Panel a shows an observed path and its controlled sign reversal; panel b shows that both paths have the same signed-domain Gramian angular summation field.}
\label{fig:main7}
\end{figure}
...
\textbf{Table 3. Absolute and relative recovery of measured wind-process structure}

The table reports the same native 10-min compound events for the validation-selected scalar comparator and ordered trajectory. RMSE values are in m s−1; relative gains use paired seven-day 95\% intervals. Compound status is defined from the power catalogue before wind evaluation.

{\def\LTcaptype{none} % do not increment counter
\begingroup\scriptsize
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.16}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.04}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.10}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.10}}
  >{\raggedright\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.18}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.14}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 14\tabcolsep) * \real{0.14}}
...
Full gain (\%)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Scalar geometry RMSE
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedleft
Ordered geometry RMSE
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Geometry gain (\%)
\end{minipage} \\
\midrule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
T11 LiDAR & 165 & 1.729 & 1.527 & 11.67 {[}4.91, 21.43{]} & 1.414 & 1.248 & 11.70 \\
T07 LiDAR & 38 & 1.092 & 0.825 & 24.45 {[}16.78, 41.87{]} & 1.003 & 0.651 & 35.12 \\
SMARTEOLE & 42 & 1.202 & 1.132 & 5.81 {[}−9.94, 7.27{]} & 0.983 & 0.820 & 16.54 \\
\end{longtable}
\endgroup
}

Here n counts compound test intervals; brackets give paired seven-day 95\% intervals for full-trajectory gains. Geometry columns give compound-event point estimates; the corresponding all-event geometry intervals and all absolute values for compound and primitive strata are provided in Supplementary Table S83. Internal geometry is the wind trajectory remaining after removal of its endpoint chord; its all-event denominators are 507, 170 and 449, respectively. Each population occupies four seven-day test blocks. Complete 3/7/14-day intervals accompany these comparisons in Supplementary Tables S75--S79.
...
\subsection{Linking historical shape to rule-based forecast costs}\label{linking-historical-shape-to-rule-based-forecast-costs}

The operational test translates forecast quality into an explicit monetary outcome. We reconstructed 15-min target points from the 33-turbine Pizhou minute archive and applied the ultra-short-term accuracy component of Jiangsu's 2022 No.~53 rule \citep{jiangsu2022accuracy}. For the provider-reported 87.45-MW farm, each forecast outside the admissible error band incurs CNY 34.98: the band is 3\% of capacity at 15 min and 13\% at 4 h. This rule-based simulation evaluates every eligible test point, so both avoided and newly introduced charges enter the total.

The information design connects the representation analysis to an operational clock. Completed two- or four-hour power prefixes are encoded as 25-point trajectories, raw principal components or angular components with protected polarity. These causal prefixes contain only pre-issue observations; they are distinct from the event-centred shapes used for physical interpretation. The longer-horizon experiment adds archived Japan Meteorological Agency (JMA) and Global Forecast System (GFS) fields with a fixed 24-h forecast lead \citep{openmeteo_previous}. Validation selects representation, weather inputs and the output action of a common probability-forest model. The selected 15-min pipeline uses raw/PCA6 from completed power history; the four-hour pipeline combines protected-polarity angular coordinates with JMA/GFS forecasts.

The selected full forecast pipeline reduces both error and accuracy charges on the chronological test period (Fig.~\ref{fig:main12}). At 15 min, charges fall from CNY 112,705.56 to CNY 108,472.98, a saving of CNY 4,232.58 or 3.76\% (95\% seven-day block interval 2.41--4.96\%). At four hours, charges fall from CNY 148,804.92 to CNY 121,695.42, saving CNY 27,109.50 or 18.22\% (15.43--21.38\%). Four-hour nMAE falls from 17.557\% to 14.167\%. The corresponding test populations contain 9,450 and 9,345 forecast targets over 16 occupied seven-day blocks. With transformations and selected settings fixed, three classifier seeds retain four-hour fee reductions of 17.51--18.22\%. Thus the gain persists across both calendar resampling and classifier randomness.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig12_policy_fees.pdf}
\caption{Cumulative simulated accuracy charges on the chronological Pizhou test calendar. Panels a and b compare persistence with the validation-selected shape pipeline at 15 min and four hours, respectively; all eligible points enter each curve. Charges use the 2022 Jiangsu ultra-short-term accuracy component at the provider-reported 87.45-MW capacity. Inset text reports full-period savings and paired seven-day block 95\% intervals. The test contains 9,450 targets at 15 min and 9,345 at four hours.}
\label{fig:main12}
\end{figure}

The comparison table places the shape pipeline alongside scalar/weather and nMAE-selected alternatives under the same test targets and capacity. All three learned approaches deliver lower charges than persistence. The 18.22\% primary result describes the complete forecast pipeline, encompassing historical information, issued weather and forecast selection. Table 4 summarizes the common-target comparison, while Supplementary Tables S59--S62 retain the information sets, matched control differences and full uncertainty analysis.

\textbf{Table 4. Rule-based accuracy charges and strong forecasting controls}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.25}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.22}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Lead
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
A fixed-model action experiment isolates why average error and cost can rank forecasts differently (Fig.~\ref{fig:main13}). With the same four-hour raw25 probability forest and persistence blend, choosing the centre of the most probable admissible band instead of the conditional mean reduces charges from CNY 139,675.14 to CNY 124,948.56. The saving is CNY 14,726.58, or 10.54\% (seven-day block interval 4.55--16.05\%), even though nMAE changes from 15.728\% to 16.769\%. The probability model, input information and test targets are held fixed. The change is the action extracted from the predictive distribution: the conditional mean targets squared error, whereas the band-centred action targets the number of charged exceedances.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{fig13_action_value.pdf}
\caption{A fixed probability model supports different forecast actions. At the four-hour lead, the raw25 model without numerical weather forecasts and its 0.75 persistence blend are held fixed. The conditional-mean action yields lower nMAE, while the admissible-band action yields lower simulated accuracy charges. The charge reduction is 10.54\% with a seven-day block 95\% interval of 4.55--16.05\%. This is a same-model action ablation, separate from the globally selected pipelines in Fig.~\ref{fig:main12} and the comparison table.}
\label{fig:main13}
\end{figure}
...
\subsection{Reference panels and forecast-to-cost experiments}\label{reference-panels-and-forecast-to-cost-experiments}
...
Two- and four-hour completed prefixes were resampled to 25 coordinates and transformed using training-only raw/PCA6, GAF/PCA6 or GAF/PCA5 plus polarity encoders. Scalar controls described recent power, wind, variation and calendar state. Archived JMA and GFS fields used the Previous Runs API's fixed 24-h-ahead forecasts, rather than target-time observations \citep{openmeteo_previous}. Including hourly interpolation, the nominal forecast vintages predated issue by more than nineteen hours at both evaluated leads. The three weather arms used common validation and test targets; model-specific archive coverage determined their training support. Supplementary S22 specifies fields, source coverage and the distinctions between this causal-prefix track and event-centred encoding.
...
Jiangsu's 2022 No.~53 Article 44(II) specified point accuracy \(A_t=100(1-|P_t-\widehat P_t|/C)\), with thresholds 97\% at 15 min and 87\% at four hours \citep{jiangsu2022accuracy}. For capacity \(C\) in MW and \(\tau_h=0.03\) or \(0.13\), we calculated the accuracy charge in Chinese yuan (CNY) as
...
The authors thank the Jiangsu Key Laboratory of Intelligent Weather Forecasting and Applications Based on Big Data for providing the experimental environment and support. This work was supported by the Interdisciplinary Research Center of Atmospheric Sciences, Nanjing University of Information Science and Technology. The authors acknowledge ENGIE Green for the SMARTEOLE field data and the SMARTEOLE project funded by the French National Research Agency (ANR-14-CE05-0034).

\section*{CRediT authorship contribution statement}
Conceptualization, P.G. and C.Y.; methodology, P.G.; software, P.G.; validation, P.G.; formal analysis, P.G.; investigation, P.G.; resources, C.Y.; data curation, P.G.; writing—original draft preparation, P.G.; writing—review and editing, C.Y.; visualization, P.G.; supervision, C.Y.; project administration, C.Y.; funding acquisition, C.Y.
